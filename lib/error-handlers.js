...
export function handleProviderError(error, currentProvider, fallbackOrder = ['openai', 'anthropic', 'meta', 'perplexity']) {
...
function getErrorMessage(error) {
    ...
    // Remove sensitive information like API keys if present
    return baseMessage.replace(/sk-[a-zA-Z0-9]+/g, '[REDACTED]')
...
export const providerErrorHandlers = {
    ...
    meta: {
        handleError: (error) => {
            // Add Meta-specific error handling
            return { retry: false };
        }
    },
    perplexity: {
        handleError: (error) => {
            // Add Perplexity-specific error handling
            return { retry: false };
        }
    }
};