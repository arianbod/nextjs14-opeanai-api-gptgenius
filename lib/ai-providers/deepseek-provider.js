...
const deepseek = new OpenAI({
    apiKey: process.env.DEEPSEEK_API_KEY, baseURL: process.env.DEEPSEEK_BASE_URL
});
...
const formatMessages = ({ persona, previousMessages, fileContent, fileName, fileType }) => {
    // Filter out only empty messages, don't truncate
...
const generateChatStream = async (messages, persona = {}) => {
    ...
    console.log("request param is:", requestParams[maxTokensParam])
    try {
        let lastError = null;
